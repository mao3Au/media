...
/*
	...
	File: king-include/king-app-admin.php
...
*/
	...
	function qa_admin_check_privileges2(&$qa_content)
/*
	Return true if user is logged in with admin privileges. If not, return false
	and set up $qa_content with the appropriate title and error message
*/
	...
	function qa_admin_language_options()
	{
		 ...
		 * See king-lang/en-GB directory for a clear example of how to use them.
		 */
	...
	function qa_admin_theme_options()
	{
		...
		foreach (glob(QA_THEME_DIR . '*', GLOB_ONLYDIR) as $directory) {
			...
			if (empty($metadata)) {
				...
				$contents = file_get_contents($directory . '/king-styles.css', false, null, -1, 8192);
...
*/
	{
		...
		$options=array(
			...
			QA_PERMIT_VERIFY => qa_lang_html('misc/permit_verify'),
		...
		if (qa_opt('enable_membership')) {
			qa_array_insert($options, QA_PERMIT_POINTS, array(QA_PERMIT_MEMBERSHIP => qa_lang_html('options/permit_membership')));
		}
...
*/
	{
		...
		if ($level>=QA_USER_LEVEL_ADMIN) {
			...
			$navigation['admin/widgets']=array(
				'label' => qa_lang_html('misc/layout_widgets'),
				'url' => qa_path_html('admin/widgets'),
			);
			$navigation['admin/membership']=array(
				'label' => qa_lang_html('misc/layout_membership'),
				'url' => qa_path_html('admin/membership'),
			);
			...
			$navigation['admin/manage']=array(
				'label' => qa_lang_html('admin/manage'),
				'url' => qa_path_html('admin/manage/'),
				'selected_on' => array('admin/manage$', 'admin/manage/users$', 'admin/manage/media$'),
			);
...
*/
	{
		...
		if ($level>=QA_USER_LEVEL_ADMIN) {
			$navigation['admin/manage']=array(
				'label' => qa_lang_html('misc/editposts'),
				'url' => qa_path_html('admin/manage/'),
			);

			$navigation['admin/manage/users']=array(
				'label' => qa_lang_html('admin/editusers_title'),
				'url' => qa_path_html('admin/manage/users'),
			);

			$navigation['admin/manage/media']=array(
				'label' => qa_lang_html('misc/media'),
				'url' => qa_path_html('admin/manage/media'),
			);
...
*/
	{
		if (file_exists(QA_INCLUDE_DIR.'king-db/install.php')) // file can be removed for extra security
			include_once QA_INCLUDE_DIR.'king-db/install.php';
...
*/
	...
	/**
	 * Retrieve metadata information from the $contents of a king-theme.php or king-plugin.php file, mapping via $fields.
	 *
