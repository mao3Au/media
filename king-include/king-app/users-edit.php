...
/*
	...
	File: king-include/king-app-users-edit.php
...
/*
	Set the user level of user $userid with $handle to $level (one of the QA_USER_LEVEL_* constraints in king-app/users.php)
...
*/
	...
	function qa_set_user_cover($userid, $imagedata, $oldblobid=null)
/*
	Set the avatar of $userid to the image in $imagedata, and remove $oldblobid from the database if not null
*/
	{
		...
		$imagedata=qa_image_constrain_data($imagedata, $width, $height, '1200');
		...
		if (isset($imagedata)) {
			...
			if (isset($newblobid)) {
				qa_db_user_set($userid, 'coverblobid', $newblobid);
