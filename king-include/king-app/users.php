...
function get_user_html( $user, $coversize = '1200', $class = null, $asize = '90' ) {
	...
	if ( $user ) {
		...
		if ( 'king-profile' === $class ) {
			require_once QA_INCLUDE_DIR . 'king-db/selects.php';
			$links = qa_db_single_select( qa_db_user_profile_selectspec($user['userid'], true) );
		...
		if ( qa_get_logged_in_level() >= QA_USER_LEVEL_ADMIN ) {
			$html .= '<button class="verify-button' . (  ( $user['verified'] ) ? ' verified' : ' nverified' ) . '" onclick="return makeVerify(this);" data-userid="' . qa_html( $user['userid'] ) . '" data-toggle="tooltip" data-placement="top" title="' . qa_lang_html( 'misc/verified' ) . '"><i class="fa fa-check-circle"></i></button>';
		} elseif ( $user['verified'] ) {
			$html .= '<div class="verify-button' . (  ( $user['verified'] ) ? ' verified' : '' ) . '" ><i class="fa fa-check-circle"></i></div>';
		}
		$html .= membership_badge($user['userid']);
		$html .= '</div>';
		...
		$html .= user_numbers( $user['userid'] );
		$html .= '<div class="user-box-buttons">';
		$html .= '<div id="follow_' . $user['userid'] . '">';
		$html .= favorite_button( $user['userid'] );
		$html .= '</div>';

		if ( qa_opt( 'allow_private_messages' ) && qa_get_logged_in_userid() && ( qa_get_logged_in_userid() != $user['userid'] ) && ! ( isset( $user['flags'] ) & QA_USER_FLAGS_NO_MESSAGES ) ) {
