...
/*
	...
	File: king-include/king-page-admin-pages.php
...
//	Get current list of pages and determine the state of this admin page
	...
	if ((qa_clicked('doaddpage') || qa_clicked('doaddlink') || qa_get('doaddlink') || qa_clicked('dosavepage')) && !isset($editpage)) {
		$editpage=array('title' => qa_get('text'), 'icon' => qa_get('icon'), 'tags' => qa_get('url'), 'nav' => qa_get('nav'), 'position' => 1);
...
//	Define an array of navigation settings we can change, option name => language key
	...
	$navoptions=array(
		...
		'nav_news' => 'main/nav_news',
		'nav_video' => 'main/nav_video',
	...
	$navoptions2=array(
		'hnav_home' => 'main/nav_home',
		'hnav_updates' => 'misc/nav_discover',
		'hnav_reactions' => 'misc/reactions',
		'hnav_shorts' => 'misc/shorts',
		'hnav_hot' => 'main/nav_hot',
		'hnav_formats' => 'misc/nav_types',	
	);
	
	$navpaths2=array(
		'hnav_home' => '',
		'hnav_updates' => 'updates',
		'hnav_reactions' => 'reactions',
		'hnav_shorts' => 'shorts',
		'hnav_hot' => 'hot',
		'hnav_formats' => 'type',	
	);
...
//	Process saving an old or new page
	...
	if (qa_clicked('docancel')) {
		...
		if (!qa_check_form_security_code('admin/pages', qa_post_text('code'))) {
			$securityexpired=true;
		} else foreach ($navoptions2 as $optionname => $langkey) {
			qa_set_option($optionname, (int)qa_post_text('option_'.$optionname));
		}
				...
				if (isset($editpage['pageid'])) { // changing existing page
					if ($isexternal)
						qa_db_page_set_fields($editpage['pageid'],
							...
							isset($errors['name']) ? $editpage['icon'] : $inicon,
					...
					if (empty($errors)) {
						if ($isexternal)
							$pageid=qa_db_page_create($inname, QA_PAGE_FLAGS_EXTERNAL | ($innewwindow ? QA_PAGE_FLAGS_NEW_WINDOW : 0), $inurl, null, null, $inpermit, $inicon);
						else {
							$pageid=qa_db_page_create($inname, 0, $inslug, $inheading, $incontent, $inpermit, $inicon);
...
//	Prepare content for theme
	...
	if (isset($editpage)) {
		...
		$qa_content['form']=array(
			...
			'fields' => array(
				...
				'icon' => array(
					'tags' => 'name="icon" id="icon"',
					'label' => qa_lang_html('misc/icon'),
					'value' => qa_html(isset($inicon) ? $inicon : @$editpage['icon']),
					'note' => 'You can find icons <a href="https://fontawesome.com/icons" target="_blank" > HERE </a>',
	...
	//	List of standard navigation links
		...
		$qa_content['form']['fields']['hnav']=array(
			'label' => qa_lang_html('admin/hnav_links_explanation'),
			'type' => 'static',
			'tight' => true,
		);
		foreach ($navoptions2 as $optionname => $langkey) {
			$qa_content['form']['fields'][$optionname]=array(
				'label' => '<a href="'.qa_path_html($navpaths2[$optionname]).'">'.qa_lang_html($langkey).'</a>',
				'tags' => 'name="option_'.$optionname.'"',
				'type' => 'checkbox',
				'value' => qa_opt($optionname),
			);
