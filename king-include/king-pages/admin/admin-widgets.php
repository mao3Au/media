...
/*
	...
	File: king-include/king-page-admin-widgets.php
...
//	Get current list of widgets and determine the state of this admin page
	...
	if (isset($widgetid)) {
		...
		if (!isset($editwidget['title'])) {
			...
			$editwidget['wtitle']=qa_get('title');
			$editwidget['wextra']='';
		}
	}
...
//	Define an array of relevant templates we can use
	...
	$templatelangkeys=array(
		'home' => 'admin/question_lists',
		'question' => 'admin/question_pages',
		'reactions' => 'misc/reactions',
		'type' => 'misc/nav_types',
		...
		'ask' => 'misc/create_image',
		'video' => 'misc/create_video',
		'news' => 'misc/create_news',
		'list' => 'misc/create_list',
		'poll' => 'misc/create_poll',
		'trivia' => 'misc/create_trivia',
		'music' => 'misc/create_music',
		'tag' => 'admin/tag_pages',
		...
		if (!qa_check_form_security_code('admin/widgets', qa_post_text('code')))
			...
			if (qa_post_text('dodelete')) {
				...
				if ($widgetfound) {
					...
					$inwtitle=qa_post_text('wtitle');	
					$inwextra = ( null !== qa_post_array('wextra') ) ? serialize(qa_post_array('wextra')) : null;
				...
				//	Perform appropriate database action
					...
					if (isset($editwidget['widgetid'])) { // changing existing widget
						...
						qa_db_widget_set_fields($widgetid, $intags, $inwtitle, $inwextra);

					} else {
						$widgetid=qa_db_widget_create($intitle, $intags, $inwtitle, $inwextra);
					}
	...
	$qa_content['form']=array(
		...
		'fields' => array(
			...
			'wtitle' => array(
				...
				'tags' => 'name="wtitle"',
				'type' => 'text',
				'value' => qa_html($editwidget['wtitle']),
			),
			'wextra' => array(
				'type' => 'custom',
				'html' => '',
...
if (isset($module) && method_exists($module, 'widget_option')) {
	$field = method_exists($module, 'widget_option') ? $module->widget_option($editwidget['wextra']): '';

	qa_array_insert($qa_content['form']['fields'], 'wextra', $field);
}
