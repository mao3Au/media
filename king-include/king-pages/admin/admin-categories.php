...
//	Work out the appropriate state for the page
	...
	$editcategory = $categories[$editcategoryid] ?? null;
		...
		//	Perform appropriate database action
			...
			if (empty($errors)) {
				if (isset($editcategory['categoryid'])) { // changing existing category
					qa_db_category_rename($editcategory['categoryid'], $inname, $inicon, $incolor, $inslug);
					...
					$categoryid=qa_db_category_create($inparentid, $inname, $inicon, $incolor, $inslug);
	...
	if ($setmissing) {
		...
		$qa_content['form']=array(
			...
			'fields' => array(
				...
				'color' => array(
					'id' => 'icon_display',
					'type' => 'custom',
					'label' => qa_lang_html('misc/color'),
					'html' => '<input type="color" id="color" name="color" value="'.qa_html(isset($incolor) ? $incolor : @$editcategory['color']).'">',					
					'error' => qa_html(@$errors['color']),
				),
				'icon' => array(
					'id' => 'icon_display',
					'tags' => 'name="icon" id="icon"',
					'label' => qa_lang_html('misc/icon'),
					'value' => qa_html(isset($inicon) ? $inicon : @$editcategory['icon']),
		...
		if ($setparent) {
			...
			$qa_content['form']['fields']['questions']=array(
				...
				'type' => 'static',
				'value' => '<a href="'.qa_path_html('home/'.qa_category_path_request($categories, $editcategory['categoryid'])).'">'.
