...
/*
	...
	File: king-include/king-page-login.php
...
//	Check we're not using KINGMEDIA's single-sign on integration and that we're not logged in
...
//	Prepare content for theme
	...
	if (! qa_opt('suspend_register_users')) {
		$rlink='<a href="'.qa_html(qa_path('register')).'" class="hllink">'.qa_lang_html('main/nav_register').'</a>';
	}
	if (qa_opt('enable_homepagelogin')) {
		$hleft = '';
		if (qa_opt('login_logo')) {
			$hleft .= '<img src="'.qa_html( qa_path_to_root() . 'king-include/watermark/' . qa_opt('login_logo') ).'" class="hlogin-logo" />';
		}
		if (qa_opt('hlogin_desc')) {
			$hleft .= '<h2 class="login-desc" style="'.( ( null !== qa_opt('desc_color') ) ? 'color:'.qa_opt('desc_color').';' : '' ).'" >'.qa_opt('hlogin_desc') .'</h2>';
		}
		if ($hleft) {
			$qa_content['custom'] = $hleft;
		}
	...
	if ( qa_opt('enable_homepagelogin') ) {
		$qa_content['hlogin']=' king-homelogin';
	}
