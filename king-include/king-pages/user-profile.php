...
/*
	...
	File: king-include/king-page-user-profile.php
...
//	Find the user profile and questions and answers for this handle
	...
	list($useraccount, $userprofile, $userfields, $usermessages, $userpoints, $userlevels, $navcategories) =
...
//	Information about user activity, available also with single sign-on integration
		...
		'fields' => array(
			...
			'points' => array(
				...
				'value' => (@$userpoints['points'] == 1)
					? qa_lang_html_sub('main/1_point', '<span class="king-uf-user-points">1</span>', '1')
					: qa_lang_html_sub('main/x_points', '<span class="king-uf-user-points">'.qa_html(number_format(@$userpoints['points'])).'</span>'),
				'id' => 'points',
			),
			...
			'questions' => array(
				...
				'value' => '<span class="king-uf-user-q-posts">'.qa_html(number_format(@$userpoints['qposts'])).'</span>',
				'id' => 'questions',
			),
			...
			'answers' => array(
				...
				'value' => '<span class="king-uf-user-a-posts">'.qa_html(number_format(@$userpoints['aposts'])).'</span>',
				'id' => 'answers',
			),
		),
	);
	...
	if (qa_opt('comment_on_qs') || qa_opt('comment_on_as')) { // only show comment count if comments are enabled
		$qa_content['form_activity']['fields']['comments'] = array(
			...
			'value' => '<span class="king-uf-user-c-posts">'.qa_html(number_format(@$userpoints['cposts'])).'</span>',
			'id' => 'comments',
		);
	}
	...
	if (qa_opt('voting_on_qs') || qa_opt('voting_on_as')) { // only show vote record if voting is enabled
		...
		if (qa_opt('voting_on_qs')) {
			...
			$innervalue = '<span class="king-uf-user-q-votes">'.number_format($qvotes).'</span>';
		...
		if (qa_opt('voting_on_as')) {
			...
			$innervalue = '<span class="king-uf-user-a-votes">'.number_format($avotes).'</span>';
		...
		$innervalue = '<span class="king-uf-user-upvotes">'.number_format($upvotes).'</span>';
		...
		$innervalue = '<span class="king-uf-user-downvotes">'.number_format($downvotes).'</span>';
		...
		$innervalue = '<span class="king-uf-user-upvoteds">'.number_format(@$userpoints['upvoteds']).'</span>';
		...
		$innervalue = '<span class="king-uf-user-downvoteds">'.number_format(@$userpoints['downvoteds']).'</span>';
	...
	if (@$userpoints['aselects']) {
		$qa_content['form_activity']['fields']['questions']['value'] .= ($userpoints['aselects'] == 1)
			? qa_lang_html_sub('profile/1_with_best_chosen', '<span class="king-uf-user-q-selects">1</span>', '1')
			: qa_lang_html_sub('profile/x_with_best_chosen', '<span class="king-uf-user-q-selects">'.number_format($userpoints['aselects']).'</span>');
	}

	if (@$userpoints['aselecteds']) {
		$qa_content['form_activity']['fields']['answers']['value'] .= ($userpoints['aselecteds'] == 1)
			? qa_lang_html_sub('profile/1_chosen_as_best', '<span class="king-uf-user-a-selecteds">1</span>', '1')
			: qa_lang_html_sub('profile/x_chosen_as_best', '<span class="king-uf-user-a-selecteds">'.number_format($userpoints['aselecteds']).'</span>');
	}
