...
/*
	...
	File: king-include/king-page-account.php
...
//	Process profile if saved
		...
		if (qa_clicked('dosaveprofile') && !$isblocked) {
			...
			if (!qa_check_form_security_code('account', qa_post_text('code')))
				...
				if (isset($_FILES['cover'])) {
					...
					// Note if $_FILES['file']['error'] === 1 then upload_max_filesize has been exceeded
					if ($coverfileerror === 1)
						$errors['cover'] = qa_lang('main/file_upload_limit_exceeded');
					elseif ($coverfileerror === 0 && $_FILES['cover']['size'] > 0) {
						require_once QA_INCLUDE_DIR.'king-app/limits.php';

						switch (qa_user_permit_error(null, QA_LIMIT_UPLOADS)) {
							case 'limit':
								$errors['cover'] = qa_lang('main/upload_limit');
								break;

							default:
								$errors['cover'] = qa_lang('users/no_permission');
								break;

							case false:
								qa_limits_increment($userid, QA_LIMIT_UPLOADS);

								$toobig = qa_image_file_too_big($_FILES['cover']['tmp_name'], qa_opt('avatar_store_size'));

								if ($toobig)
									$errors['cover'] = qa_lang_sub('main/image_too_big_x_pc', (int) ($toobig * 100));
								elseif (!qa_set_user_cover($userid, file_get_contents($_FILES['cover']['tmp_name']), $useraccount['coverblobid']))
									$errors['cover'] = qa_lang_sub('main/image_not_read', implode(', ', qa_gd_image_formats()));
								break;
						}
					}  // There shouldn't be any need to catch any other error
				}
	...
	//	Process change password if clicked
		...
		if (qa_clicked('delmyaccount') && qa_check_form_security_code('delaccount', qa_post_text('code'))) {
			require_once QA_INCLUDE_DIR.'king-app/users-edit.php';
			qa_delete_user($useraccount['userid']);
			qa_redirect('logout');
		}
	}
...
//	Prepare content for theme
	...
	$qa_content['form_profile']=array(
		...
		'fields' => array(
			...
			'cover' => array(
				'label' => qa_lang_html('misc/cover'),
				'type' => 'custom',
				'html' => '<span class="king-cover-ac">'.qa_get_avatar_blob_html($useraccount['coverblobid'], $useraccount['avatarwidth'], $useraccount['avatarheight'], 100).'</span><input name="cover" type="file">',
				'error' => qa_html(@$errors['cover']),
			),
	...
	if (qa_opt('avatar_allow_upload')) {
		...
		if (qa_opt('avatar_default_show') && strlen(qa_opt('avatar_default_blobid'))) {
			$avataroptions['']='<span style="margin:2px 0; display:inline-block;">'.
				qa_get_avatar_blob_html(qa_opt('avatar_default_blobid'), qa_opt('avatar_default_width'), qa_opt('avatar_default_height'), 52).
		...
		if (qa_has_gd_image() && qa_opt('avatar_allow_upload')) {
			...
			if (isset($useraccount['avatarblobid']))
				$avataroptions['uploaded']='<span style="margin:2px 0; display:inline-block;">'.
					qa_get_avatar_blob_html($useraccount['avatarblobid'], $useraccount['avatarwidth'], $useraccount['avatarheight'], 52).
...
//	Change password form
	...
	$qa_content['form_delaccount']=array(
		'tags' => 'method="post" action="'.qa_self_html().'"',

		'style' => 'wide',

		'title' => qa_lang_html('misc/delete_myaccount'),

		'buttons' => array(
			'change' => array(
				'label' => qa_lang_html('misc/delete'),
			),
		),

		'hidden' => array(
			'delmyaccount' => '1',
			'code' => qa_get_form_security_code('delaccount'),
		),
	);
	if (!$haspassword) {
