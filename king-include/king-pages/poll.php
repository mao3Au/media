...
if ($permiterror || qa_opt( 'disable_poll' )) {
	...
	if (!qa_check_form_security_code('ask', qa_post_text('code'))) {
		...
		if (empty($errors)) {
			...
			$questionid = qa_question_create($followanswer, $userid, qa_get_logged_in_handle(), $cookieid,
				...
				$in['notify'], $in['email'], $in['categoryid'], $in['extra'], $in['queued'], $in['name'], 'poll', $in['pcontent'], $in['nsfw']);
			if ( isset( $_POST['out'] ) ) {
				foreach ( $_POST['out'] as $row => $value ) {
					$in['poll']  = $_POST['out'][$row];
					$poll  = serialize( $in['poll'] );
					$pollgrid = qa_gpc_to_string($_POST['grid'][$row]);
					qa_db_query_sub('INSERT INTO ^poll (postid, content, extra, created) VALUES (#, $, $, NOW())', $questionid, $poll, $pollgrid );
				}
			} else {
				$in['poll'] = null;
			}
...
$qa_content['title'] = qa_lang_html(isset($followanswer) ? 'question/ask_follow_title' : 'misc/king_poll');
...
$poll   = '<div class="king-ang" ng-repeat="column in columns track by $index | limitTo:24">
	...
	<div class="inputarea">
		...
		<input class="hide" type="text" ng-model="columnimg" name="out[{{$index+1}}][pimg]" autocomplete="off" maxlength="40"/>
		<div ui-sortable="sortableOptions2" ng-model="column.inputs" class="king-poll-grids {{polltab}}">
			<div ng-repeat="input in column.inputs track by $index | limitTo:24" class="king-poll-grid">
				...
				<input class="hide" type="text" ng-model="input.img" ng-show="polltab !== \'grid1\'" name="out[{{$parent.$index+1}}][pa][{{$index+1}}][img]" autocomplete="off" maxlength="40"/>
