...
/*
	...
	File: king-include/king-feed.php
...
//	Prepare the XML output
	...
	$lines[]='<description>Powered by King-Media</description>';
	...
	//	Determine whether this is a question, answer or comment, and act accordingly
		...
		if (isset($question['opostid'])) {
			...
			if ($full)
				$htmlcontent=qa_viewer_html($question['pcontent'], $question['format'], $options);
		}
	...
	//	Build the inner XML structure for each item
		...
		if (isset($htmlcontent)) {
			...
			$cont = isset($question['content']) ? king_get_uploads( $question['content'] ) : '';


			$lines[]='<description>';
			if (isset($cont['furl'])) {
				$lines[].='<figure><img src="'.qa_html($cont['furl']).'" width="' . $cont['width'] . '" height="' . $cont['height'] . '"/></figure>';
			}
			
			$lines[].= ''.$htmlcontent.'</description>';
		}
