...
File: king-include/king-base.php
Description: Sets up KINGMEDIA environment, plus many globally useful functions
...
define('QA_VERSION', '8.2'); // also used as suffix for .js and .css requests
define('QA_BUILD_DATE', '2022-10-01');

/**
 * Autoloads some KINGMEDIA classes so it's possible to use them without adding a require_once first. From version 1.7 onwards.
 ...
 * for that matter.
 *
 * Classes are stored in the king-include/KINGMEDIA folder, and then in subfolders depending on their categorization.
 * Class names should be of the form KINGMEDIA_<Namespace>_<Class>, e.g. KINGMEDIA_Util_Debug. There may be multiple "namespaces".
 * Classes are mapped to PHP files with the underscores converted to directory separators. The KINGMEDIA_Util_Debug class is in
 * the file king-include/KINGMEDIA/king-util/Debug.php. A class named KINGMEDIA_Db_User_Messages would be in a file king-include/KINGMEDIA/king-db/User/Messages.php.
 */
function qa_autoload($class)
{
    if (strpos($class, 'KINGMEDIA_') === 0) {
...
if (!function_exists('json_encode') && !function_exists('json_decode')) {
    require_once QA_INCLUDE_DIR . 'king-vendor/JSON.php';
 ...
 */
{
    ...
    if (preg_match('/[0-9\.]+/', (string)$version, $matches)) {
...
/*
Returns true if the current KINGMEDIA version is lower than $version, if both are valid version strings for qa_version_to_float()
 */
{
...
/*
Set up and verify the PHP environment for KINGMEDIA, including unregistering globals if necessary
 */
{
    if (qa_php_version_below('5.1.6')) {
        qa_fatal_error('KINGMEDIA requires PHP 5.1.6 or later');
    }
...
/*
First stage of setting up KINGMEDIA constants, before (if necessary) loading WordPress integration
 */
{
    ...
    // try out best if not set in index.php or king-index.php - won't work with symbolic links

    define('QA_EXTERNAL_DIR', QA_BASE_DIR . 'king-external/');
    define('QA_INCLUDE_DIR', QA_BASE_DIR . 'king-include/');
    define('QA_LANG_DIR', QA_BASE_DIR . 'king-lang/');
    define('QA_THEME_DIR', QA_BASE_DIR . 'king-theme/');
    define('QA_PLUGIN_DIR', QA_BASE_DIR . 'king-plugin/');

    if (!file_exists(QA_BASE_DIR . 'king-config.php')) {
        qa_fatal_error('The config file could not be found. Please read the instructions in king-config-example.php.');
    }
    $QA_CONST_PATH_MAP=array(
    'home' => '',
    'ask' => 'submit',
    );
    require_once QA_BASE_DIR . 'king-config.php';
    ...
    if (defined('QA_WORDPRESS_INTEGRATE_PATH') && strlen(QA_WORDPRESS_INTEGRATE_PATH)) {
        ...
        if (!is_readable(QA_WORDPRESS_LOAD_FILE)) {
            qa_fatal_error('Could not find wp-load.php file for WordPress integration - please check QA_WORDPRESS_INTEGRATE_PATH in king-config.php');
        }
...
/*
Second stage of setting up KINGMEDIA constants, after (if necessary) loading WordPress integration
 */
{

    //    Default values if not set in king-config.php
    ...
    //    Possible URL schemes for KINGMEDIA and the string used for url scheme testing
...
/*
Register all modules that come as part of the KINGMEDIA core (as opposed to plugins)
 */
{
    qa_register_module('filter', 'king-addons/king-filter-basic.php', 'qa_filter_basic', '');
    qa_register_module('editor', 'king-addons/king-editor-basic.php', 'qa_editor_basic', '');
    qa_register_module('viewer', 'king-addons/king-viewer-basic.php', 'qa_viewer_basic', '');
    qa_register_module('event', 'king-addons/king-event-limits.php', 'qa_event_limits', 'KINGMEDIA Event Limits');
    qa_register_module('event', 'king-addons/king-event-notify.php', 'qa_event_notify', 'KINGMEDIA Event Notify');
    qa_register_module('event', 'king-addons/king-event-updates.php', 'qa_event_updates', 'KINGMEDIA Event Updates');
    qa_register_module('search', 'king-addons/king-search-basic.php', 'qa_search_basic', '');
    qa_register_module('widget', 'king-addons/king-widget-related-qs.php', 'qa_related_qs', 'Related Posts');
    qa_register_module('widget', 'king-addons/king-widget-cats.php', 'king_cats', 'Categories');
    qa_register_module('widget', 'king-addons/king-widget-formats.php', 'king_postformats', 'Recent Post Formats');
    qa_register_module('widget', 'king-addons/king-tag-cloud.php', 'king_tag_cloud', 'Tag Clouds');
    qa_register_module('widget', 'king-addons/king-widget-ad.php', 'king_ad', 'Advertisement widget');
    qa_register_module('widget', 'king-addons/king-widget-hot.php', 'king_hotposts', 'Hot Posts widget');
}

/**
 * Retrieve metadata information from the $contents of a king-theme.php or king-plugin.php file, specified by $type ('Plugin' or 'Theme').
 * If $versiononly is true, only min version metadata is parsed.
 * Name, Description, Min KINGMEDIA & Min PHP are not currently used by themes.
 *
 * @deprecated Deprecated from 1.7; KINGMEDIA_Util_Metadata class and metadata.json files should be used instead
 */
function qa_addon_metadata($contents, $type, $versiononly = false)
{
    $fields = array(
        'min_KINGMEDIA' => 'Minimum King-Media Version',
...
/*
Load all the king-plugin.php files from plugins that are compatible with this version of KINGMEDIA
 */
{
    global $qa_plugin_directory, $qa_plugin_urltoroot;

    $pluginfiles = glob(QA_PLUGIN_DIR . '*/king-plugin.php');

    $metadataUtil = new KINGMEDIA_Util_Metadata();
    foreach ($pluginfiles as $pluginfile) {
        ...
        // skip plugin which requires a later version of KINGMEDIA
        if (isset($metadata['min_KINGMEDIA']) && qa_qa_version_below($metadata['min_KINGMEDIA'])) {
            continue;
        }
...
//    Functions for registering different varieties of KINGMEDIA modularity
...
the language code (e.g. 'fr') and/or 'default'. These files should be formatted like KINGMEDIA's king-lang-*.php files.
 */
{
    global $qa_lang_file_pattern;

    if (file_exists(QA_INCLUDE_DIR . 'lang/king-lang-' . $name . '.php')) {
...
//    Function for registering varieties of KINGMEDIA modularity, which are (only) called from king-plugin.php files
...
This function relies on some global variable values and can only be called from a plugin's king-plugin.php file
 */
{
    global $qa_plugin_directory, $qa_plugin_urltoroot;

    if (empty($qa_plugin_directory) || empty($qa_plugin_urltoroot)) {
        qa_fatal_error('qa_register_plugin_module() can only be called from a plugin king-plugin.php file');
    }
...
/*
Register a plugin layer named $name, defined in file $include. Can only be called from a plugin's king-plugin.php file
 */
{
    global $qa_plugin_directory, $qa_plugin_urltoroot;

    if (empty($qa_plugin_directory) || empty($qa_plugin_urltoroot)) {
        qa_fatal_error('qa_register_plugin_layer() can only be called from a plugin king-plugin.php file');
    }
...
/*
Register a plugin file $include containing override functions. Can only be called from a plugin's king-plugin.php file
 */
{
    global $qa_plugin_directory, $qa_plugin_urltoroot;

    if (empty($qa_plugin_directory) || empty($qa_plugin_urltoroot)) {
        qa_fatal_error('qa_register_plugin_overrides() can only be called from a plugin king-plugin.php file');
    }
 ...
 */
{
    ...
    if (empty($qa_plugin_directory) || empty($qa_plugin_urltoroot)) {
        qa_fatal_error('qa_register_plugin_phrases() can only be called from a plugin king-plugin.php file');
    }

    qa_register_phrases($qa_plugin_directory . $pattern, $name);
}

//    Low-level functions used throughout KINGMEDIA
...
function qa_fatal_error($message)
{
    ...
    echo 'King-Media fatal error:<p style="color: red">' . qa_html($message, true) . '</p>';
    @error_log('PHP King-Media fatal error: ' . $message);
    echo '<p>Stack trace:<p>';

    $backtrace = array_reverse(array_slice(debug_backtrace(), 1));
    foreach ($backtrace as $trace) {
        $color = strpos($trace['file'] ?? '', '/king-plugin/') !== false ? 'red' : '#999';
        echo sprintf(
 ...
 */
{
    ...
    require_once 'king-vendor/htmLawed.php';
...
/*
Inform KINGMEDIA that the current request is $request (slash-separated, independent of the url scheme chosen),
that the relative path to the KINGMEDIA root apperas to be $relativeroot, and the url scheme appears to be $usedformat
 */
{
    if (qa_to_override(__FUNCTION__)) {$args = func_get_args();return qa_call_override(__FUNCTION__, $args);}
...
function qa_request()
/*
Returns the current KINGMEDIA request (slash-separated, independent of the url scheme chosen)
 */
{
    if (qa_to_override(__FUNCTION__)) {$args = func_get_args();return qa_call_override(__FUNCTION__, $args);}
...
/*
Returns the indexed $part (as separated by slashes) of the current KINGMEDIA request, or null if it doesn't exist
 */
{
...
/*
Returns an array of parts (as separated by slashes) of the current KINGMEDIA request, starting at part $start
 */
{
 ...
 * $identifier before the slash (/) replaces the * in the king-lang-*.php file references, and the
...
function qa_lang($identifier)
{
    ...
    //    First look for a custom phrase in king-lang/custom/

    if (!isset($qa_phrases_custom[$group])) {
        // only load each language file once
        $phrases                   = @include QA_LANG_DIR . 'custom/king-lang-' . $group . '.php'; // can tolerate missing file or directory
    ...
    //    Second look for a localized file in king-lang/<lang>/

  $languagecode = qa_get_lang();
    ...
    if (strlen($languagecode)) {
        if (!isset($qa_phrases_lang[$group])) {
            if (isset($qa_lang_file_pattern[$group])) {
                ...
                $include = QA_LANG_DIR . $languagecode . '/king-lang-' . $group . '.php';
            }
    ...
    if (!isset($qa_phrases_default[$group])) {
        // only load each default language file once
        if (isset($qa_lang_file_pattern[$group])) {
            $include = str_replace('*', 'default', $qa_lang_file_pattern[$group]);
        } else {
            $include = QA_INCLUDE_DIR . 'lang/king-lang-' . $group . '.php';
        }
...
/*
Return the relative path to the KINGMEDIA root (if it was previously set by qa_set_request())
 */
{
    if (qa_to_override(__FUNCTION__)) {$args = func_get_args();return qa_call_override(__FUNCTION__, $args);}
...
/*
Return an array of mappings of KINGMEDIA requests, as defined in the king-config.php file
 */
{
    if (qa_to_override(__FUNCTION__)) {$args = func_get_args();return qa_call_override(__FUNCTION__, $args);}
...
that as the root of the KINGMEDIA site, otherwise use path to root which was set elsewhere.
 */
{
    if (qa_to_override(__FUNCTION__)) {$args = func_get_args();return qa_call_override(__FUNCTION__, $args);}
...
/*
Return the KINGMEDIA request for question $questionid, and make it search-engine friendly based on $title, which is
