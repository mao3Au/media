...
*/
	...
	/**
	 * Indicates to the KINGMEDIA database layer that database connections are permitted fro this point forwards
	...
	/**
	 * Connect to the KINGMEDIA database, select the right database, optionally install the $failhandler (and call it if necessary).
	 * Uses mysqli as of KINGMEDIA 3.
	 */
	function qa_db_connect($failhandler=null)
	{
		...
		if (!$qa_db_allow_connect)
			qa_fatal_error('It appears that a plugin is trying to access the database, but this is not allowed until KINGMEDIA initialization is complete.');
		...
		// From KINGMEDIA 1.5, we explicitly set the character encoding of the MySQL connection, instead of using lots of "SELECT BINARY col"-style queries.
...
function qa_db_fail_error($type, $errno = null, $error = null, $query = null)
{
	...
	@error_log('PHP King-Media MySQL ' . $type . ' error ' . $errno . ': ' . $error . (isset($query) ? (' - Query: ' . $query) : ''));

	if (isset($qa_db_fail_handler) && function_exists($qa_db_fail_handler))
	...
	/**
	 * Return the current connection to the KINGMEDIA database, connecting if necessary and $connect is true.
	 */
	...
	/**
	 * Disconnect from the KINGMEDIA database.
	 */
...
function qa_db_query_execute($query)
{
	...
	/**
	 * Return $string escaped for use in queries to the KINGMEDIA database (to which a connection must have been made).
	 */
	...
	/**
	 * Return an array of the names of all tables in the KINGMEDIA database, converted to lower case.
	 * No longer used by KINGMEDIA and shouldn't be needed.
	 */
	...
	/**
	 * Return an array of the names of all tables in the KINGMEDIA database.
	 */
