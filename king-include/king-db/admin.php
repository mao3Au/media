...
/*
	...
	File: king-include/king-db-admin.php
...
/*
	Return the total size in bytes of all relevant tables in the KINGMEDIA database
*/
	{
...
*/
	...
	function qa_db_category_create($parentid, $title, $icon='', $color='', $tags='')
/*
...
*/
	{
		...
		qa_db_query_sub(
			'INSERT INTO ^categories (parentid, title, tags, position, icon, color) VALUES ($, $, $, #, $, $)',
			$parentid2, $title, $tags, 1+$lastpos, $icon, $color
		);
...
*/
	...
	function qa_db_category_rename($categoryid, $title, $icon='', $color='', $tags='')
/*
	Set the name of $categoryid to $title and its slug to $tags in the database
*/
	{
		qa_db_query_sub(
			'UPDATE ^categories SET title=$, icon=$, color=$, tags=$ WHERE categoryid=#',
			$title, $icon, $color, $tags, $categoryid
		);
...
*/
	...
	function qa_db_page_create($title, $flags, $tags, $heading, $content, $permit=null, $icon=null)
/*
...
*/
	{
		...
		qa_db_query_sub(
			'INSERT INTO ^pages (title, icon, nav, flags, permit, tags, heading, content, position) VALUES ($, $, \'\', #, #, $, $, $, #)',
			$title, $icon, $flags, $permit, $tags, $heading, $content, $position
		);
	...
	function qa_db_page_set_fields($pageid, $title, $icon, $flags, $tags, $heading, $content, $permit=null)
/*
	Set the fields of $pageid to the values provided in the database
*/
	{
		qa_db_query_sub(
			'UPDATE ^pages SET title=$, icon=$, flags=#, permit=#, tags=$, heading=$, content=$ WHERE pageid=#',
			$title, $icon, $flags, $permit, $tags, $heading, $content, $pageid
		);
	}
...
*/
	...
	function qa_db_widget_create($title, $tags, $wtitle = null, $wextra = null)
/*
...
*/
	{
		...
		qa_db_query_sub(
			'INSERT INTO ^widgets (place, position, tags, title, wtitle, wextra) VALUES (\'\', #, $, $, $, $)',
			$position, $tags, $title, $wtitle, $wextra
		);
	...
	function qa_db_widget_set_fields($widgetid, $tags, $wtitle = null, $wextra = null)
/*
	Set the comma-separated list of templates for $widgetid to $tags
*/
	{
		qa_db_query_sub(
			'UPDATE ^widgets SET tags=$, wtitle=$, wextra=$ WHERE widgetid=#',
			$tags, $wtitle, $wextra, $widgetid
		);
	}
