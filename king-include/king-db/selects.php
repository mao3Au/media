...
File: king-include/king-db/selects.php
Description: Builders of selectspec arrays (see king-db.php) used to specify database SELECTs
 ...
 */
{
	...
	$selectspec = array(
		'columns'   => array(
			'^posts.postid', '^posts.content', '^posts.pcontent', '^posts.categoryid', '^posts.type', 'basetype' => 'LEFT(^posts.type, 1)', 'hidden' => "INSTR(^posts.type, '_HIDDEN')>0",
			'^posts.acount', '^posts.selchildid', '^posts.closedbyid', '^posts.upvotes', '^posts.downvotes', '^posts.netvotes', '^posts.views', '^posts.hotness',
			'^posts.flagcount', '^posts.title', '^posts.postformat', '^posts.nsfw', '^posts.featured', '^posts.tags', 'created'     => 'UNIX_TIMESTAMP(^posts.created)', '^posts.name',
 ...
 */
{
	...
	switch ($by) {
		...
		case 'news':
			...
			break;
		case 'poll':
			$bysql = "postformat='poll'";
			break;
		case 'list':
			$bysql = "postformat='list'";
			break;
		case 'trivia':
			$bysql = "postformat='trivia'";
			break;
		case 'music':
			$bysql = "postformat='music'";
			break;
		default:
			$bysql = "postformat='V'";
			break;
...
function get_shorts($voteuserid, $count = null)
{
	...
	$selectspec['source'] .= ' JOIN ^postmetas AS pmetas ON ^posts.postid=pmetas.postid WHERE pmetas.title=$ AND pmetas.content > 0 AND ^posts.type="Q" AND ^posts.postformat="V" ORDER BY ^posts.created DESC LIMIT #';

	array_push($selectspec['arguments'], 'qa_q_extra', $count);
 ...
 */
{
	...
	function post_bookmark( $postid, $class=null ) {
		...
		if ($userid) {
			...
			$aclass = '';
			if ( is_array( $result ) && in_array( $postid, $result ) ) {
				$aclass = ' selected';
			}
			if ($class) {
				$cl = $class;
			...
			$out = '<a href="#" class="' . qa_html( $cl ) . qa_html( $aclass ) . '" data-toggle="tooltip" data-placement="right" title="' . qa_lang_html( 'misc/king_bookmark' ) . '" data-bookmarkid="' . qa_html( $postid ) . '" onclick="return bookmark(this);">
...
function king_insert_uploads($path, $format, $width = null, $height = null, $type = null)
{
	qa_db_query_sub('INSERT INTO ^uploads (content, format, width, height, type) VALUES ($, $, #, #, $)',
		$path, $format, $width, $height, $type
	);
	return qa_db_last_insert_id();
}

function king_delete_uploads($id)
{
	qa_db_query_sub('DELETE FROM ^uploads WHERE id=#', $id);
	return null;
}
...
function king_get_uploads($upid)
{
...
function qa_db_media_selectspec($start = null, $count = null)
{
	$arguments = array();
	$source = '^uploads WHERE type IS NULL AND ( height IS NULL OR width=800 ) ORDER BY id ASC';
	if (isset($start)) {
		$source .= ' LIMIT #,#';
		$arguments[] = $start;
		$arguments[] = $count;
	...
	return array(
		'columns'   => array('id', 'content', 'format', 'width', 'height'),
		'source'    => $source,
		'arguments' => $arguments,
	);
}
function king_insert_membership($type, $usd, $userid, $receipt = null, $receipt_url = null)
{

	require_once QA_INCLUDE_DIR . 'king-db/metas.php';


	$checkid = qa_db_read_one_assoc(qa_db_query_sub('SELECT userid FROM ^membership WHERE receipt=$', $receipt), true);
	$now = date( 'Y-m-d' );
	$tme = qa_opt('plan_n_'.$type) . ' ' .qa_opt('plan_t_'.$type);
	
	if ( 'unlimited' === qa_opt('plan_t_'.$type) ) {
		$outdate = date( 'Y-m-d', strtotime( '20 years' ) );
	} else {
		$outdate = date( 'Y-m-d', strtotime( $tme ) );
	}
	
	if ( ! $checkid ) {
		qa_db_query_sub('INSERT INTO ^membership (userid, type, usd, created, outdate, receipt, receipt_url) VALUES (#, #, #, $, $, $, $)',
		$userid, $type, $usd, $now, $outdate, $receipt, $receipt_url );
		...
		if ($odate) {
			...
			if ( 'unlimited' === qa_opt('plan_t_'.$type) ) {
				$outdate2 = date( 'Y-m-d', strtotime( '20 years' ) );
			} else {
				$outdate2 = date( 'Y-m-d', strtotime( $date . $tme ) );
			}
			qa_db_usermeta_set( $userid, 'membership', $outdate2 );
		} else {
			qa_db_usermeta_set( $userid, 'membership', $outdate );
		}
		qa_db_usermeta_set( $userid, 'membership_plan', $type );
		if (qa_opt('ailimits')) {
			qa_db_usermeta_set( $userid, 'ailmt', 0 );
		}
	}
	return qa_db_last_insert_id();
}

function chekmembership($userid) {
	require_once QA_INCLUDE_DIR . 'king-db/metas.php';
	$odate = qa_db_usermeta_get($userid, 'membership');
	$now = date( 'Y-m-d' );
	if ( $now >= $odate ) {
...
function membership_badge($userid) {

	if ( qa_opt('enable_membership') && chekmembership($userid) ) {
		$html = '<span class="membership-badge" data-toggle="tooltip" data-placement="top" title="' . qa_lang_html( 'misc/membership_badge' ) . '"><i class="fa-solid fa-fingerprint"></i></span>';
		return $html;
	} else {
		return null;
	}
...
function get_poll($pid, $type='', $one=null)
{
	if ($one) {
		if (!$type) {
			return qa_db_read_one_assoc( qa_db_query_sub( "SELECT * FROM ^poll WHERE postid=$", $pid ), true );
		} else {
			return qa_db_read_one_assoc( qa_db_query_sub( "SELECT * FROM ^poll WHERE postid=$ AND type=$ ", $pid, $type ), true );
		}
	} elseif ($type) {
		return qa_db_read_all_assoc( qa_db_query_sub( "SELECT * FROM ^poll WHERE postid=$ AND type=$ ", $pid, $type ) );
	} else {
		return qa_db_read_all_assoc( qa_db_query_sub( "SELECT * FROM ^poll WHERE postid=$", $pid ) );
	}
	
	
}
function get_avatar($userblobid, $size='200', $url=null, $class='king-avatar') {


	if ($userblobid) {
		$avatar = qa_path_html('image', array('qa_blobid' => $userblobid, 'qa_size' => $size+80), qa_opt('site_url'), QA_URL_FORMAT_PARAMS);
		if ($url) {
			$html = $avatar;
		} else {
			$html = '<img data-king-img-src="'.$avatar.'" class="'.$class.' king-lazy" width="'.$size.'" height="'.$size.'"/>';
		}
	} elseif(qa_opt('avatar_default_blobid')) {
		$userblobid = qa_opt('avatar_default_blobid');
		$avatar = qa_path_html('image', array('qa_blobid' => $userblobid, 'qa_size' => $size+80), qa_opt('site_url'), QA_URL_FORMAT_PARAMS);
		if ($url) {
			$html = $avatar;
		} else {
			$html = '<img src="'.$avatar.'" width="'.$size.'" height="'.$size.'" class="'.$class.'" />';
		}
	} else {
		if ($url) {
			$html = '';
		} else {
			$html = '<div class="'.$class.' king-noavatar" style="height:'.$size.'px;width:'.$size.'px;"><i class="fas fa-user"></i></div>';
		}
 ...
 */
{
	...
	$selectspec = array(
		'columns'   => array('^categories.categoryid', '^categories.parentid', 'title' => '^categories.title', 'tags' => '^categories.tags', '^categories.qcount', '^categories.position', '^categories.icon', '^categories.color'),
...
{
	$selectspec = array(
		'columns'  => array('pageid', 'title', 'icon', 'flags', 'permit' => 'permit+0', 'nav', 'tags', 'position', 'heading'),
...
{
	return array(
		'columns' => array('widgetid', 'place', 'position', 'tags', 'title', 'wtitle', 'wextra'),
...
{
	return array(
		'columns'   => array('pageid', 'title', 'icon', 'flags', 'permit', 'nav', 'tags', 'position', 'heading', 'content'),
 ...
 */
{
	return array(
		'columns'   => array(
			...
			'created'      => 'UNIX_TIMESTAMP(created)', 'sessioncode', 'sessionsource', 'flags', 'verified', 'loggedin' => 'UNIX_TIMESTAMP(loggedin)',
			...
			'coverblobid'  => 'BINARY coverblobid', // cast to BINARY due to MySQL bug which renders it signed in a union
 ...
 */
{
	...
	if (QA_FINAL_EXTERNAL_USERS) {
		...
		return array(
			'columns'   => array('^users.userid', 'handle', 'points', 'level', 'flags', 'verified', '^users.email', 'coverblobid' => 'BINARY coverblobid', 'avatarblobid' => 'BINARY avatarblobid', 'avatarwidth', 'avatarheight'),
...
{
	return array(
		'columns'   => array('^users.userid', 'handle', 'level', 'coverblobid', 'avatarblobid', 'verified'),
 ...
 */
{
	...
	return array(
		'columns'   => array('^users.userid', 'handle', 'flags', 'verified', 'level', 'coverblobid', 'avatarblobid'),
		'source'    => $source,
		'arguments' => $arguments,
	);
}
...
function king_db_get_reaction_posts($voteuserid, $start = 0, $limit = null, $type = null)
/*
Return the selectspec to retrieve an array of $userid's favorited questions, with the usual information.
Returns $limit questions, or all of them if $limit is null (used in qa_db_selectspec_count).
 */
{
	...
	$selectspec['source'] .= ' JOIN ^postmetas AS pmetas ON ^posts.postid=pmetas.postid WHERE pmetas.title=$ AND ^posts.type="Q" ORDER BY pmetas.content DESC';
	$selectspec['arguments'][] = $type;
...
function qa_db_user_FOLLOWER_users_selectspec($userid, $limit = null, $start = 0)
/*
Return the selectspec to retrieve an array of $userid's favorited users, with information about those users' accounts.
Returns $limit users, or all of them if $limit is null (used in qa_db_selectspec_count).
 */
...
function qa_db_user_updates_selectspec($userid, $limit = null, $start = 0)
/*
...
favorited and which no longer post to user streams (see long comment in king-db-favorites.php).
 */
{
	...
	$selectspec['source'] .= ' JOIN ^userfavorites AS selectfave ON ^posts.userid=selectfave.entityid WHERE selectfave.userid=$ AND selectfave.entitytype=$ AND ^posts.type="Q" ORDER BY ^posts.created DESC';
	$selectspec['arguments'][] = $userid;
	$selectspec['arguments'][] = QA_ENTITY_USER;
 ...
 */
{
	...
	function king_following_tags($userid, $limit=null, $start=0, $tags=null)
	{

		$selectspec = qa_db_posts_basic_selectspec($userid);

		$selectspec['source'] .= ' JOIN ^posttags AS pt ON ^posts.postid=pt.postid WHERE pt.wordid IN(' . implode(',', $tags) . ') AND ^posts.type="Q" ORDER BY ^posts.created DESC';
	if (isset($limit)) {
		$limit = min($limit, QA_DB_RETRIEVE_QS_AS);
		$selectspec['source'] .= ' LIMIT #,#';
		$selectspec['arguments'][] = $start;
	...
	function king_following_cats($userid, $limit=null, $start=0, $tags=null)
	{

		$selectspec = qa_db_posts_basic_selectspec($userid);

		$selectspec['source'] .= ' WHERE ^posts.categoryid IN(' . implode(',', $tags) . ') AND ^posts.type=$ ORDER BY ^posts.created DESC';
		if (isset($limit)) {
			$limit = min($limit, QA_DB_RETRIEVE_QS_AS);
			$selectspec['source'] .= ' LIMIT #,#';
			$selectspec['arguments'][] = 'Q';
			$selectspec['arguments'][] = $start;
			$selectspec['arguments'][] = $limit;
		}
	...
	function king_following_cats2($userid, $limit=null, $start=0, $tags=null)
	{

		$selectspec = qa_db_posts_basic_selectspec($userid);

		$selectspec['source'] .= ' WHERE ^posts.catidpath1 IN(' . implode(',', $tags) . ') AND ^posts.type=$ ORDER BY ^posts.created DESC';
		if (isset($limit)) {
			$limit = min($limit, QA_DB_RETRIEVE_QS_AS);
			$selectspec['source'] .= ' LIMIT #,#';
			$selectspec['arguments'][] = 'Q';
...
function get_tagname_byid($tagid)
/*
Return the selectspec to retrieve the number of questions tagged with $tag (single value)
 */
{
