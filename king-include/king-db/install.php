...
/*
	...
	File: king-include/king-db-install.php
...
*/
	...
	/*
		...
		[this note is no longer relevant since we *do* explicitly set the connection character set since KINGMEDIA 1.5 - see king-db.php
	*/
	...
	/*
		...
		* See box at top of king-app-recalc.php for a list of redundant (non-normal) information in the database
		...
		* Starting in version 1.2, we explicitly name keys and foreign key constraints, instead of allowing MySQL
		  ...
		  indeed *did* assign for people who installed an earlier version of KINGMEDIA. By naming them explicitly, we're
		...
		* We name some columns here in a not entirely intuitive way. The reason is to match the names of columns in
		  ...
		  queries together via a UNION in qa_db_multi_select() - see comments in king-db.php for more information.
	*/
		...
		$tables=array(
			'users' => array(
				...
				'coverblobid' => 'BIGINT UNSIGNED', // blobid of stored avatar
				...
				'flags' => 'SMALLINT UNSIGNED NOT NULL DEFAULT 0', // see constants at top of king-app/users.php
			...
			'userlevels' => array(
				...
				'entitytype' => "CHAR(1) CHARACTER SET ascii NOT NULL", // see king-app-updates.php
			...
			'userfields' => array(
				...
				'flags' => 'TINYINT UNSIGNED NOT NULL', // QA_FIELD_FLAGS_* at top of king-app/users.php
			...
			'userevents' => array(
				...
				'updatetype' => 'CHAR(1) CHARACTER SET ascii', // what was done to this part - see king-app-updates.php
			...
			'categories' => array(
				...
				'icon' => 'VARCHAR('.QA_DB_MAX_CAT_PAGE_TITLE_LENGTH.') NOT NULL DEFAULT \'\'', // category name
				'color' => 'VARCHAR('.QA_DB_MAX_CAT_PAGE_TITLE_LENGTH.') NOT NULL DEFAULT \'\'', // category name
			...
			'pages' => array(
				...
				'icon' => 'VARCHAR('.QA_DB_MAX_TITLE_LENGTH.')',
			...
			'widgets' => array(
				...
				'wextra' => 'VARCHAR(8000)', // remainder of page HTML
				'wtitle' => 'VARCHAR('.QA_DB_MAX_WIDGET_TITLE_LENGTH.')', // name of widget module that should be displayed
			...
			'posts' => array(
				...
				'postformat' => 'VARCHAR('.QA_DB_MAX_NAME_LENGTH.')',
				...
				'updatetype' => 'CHAR(1) CHARACTER SET ascii', // see king-app-updates.php
				'title' => 'VARCHAR('.QA_DB_MAX_TITLE_LENGTH.')',
				'content' => 'VARCHAR('.QA_DB_MAX_CONTENT_LENGTH.')',
				'pcontent' => 'VARCHAR('.QA_DB_MAX_CONTENT_LENGTH.')',
				'nsfw' => 'TINYINT UNSIGNED',
				'featured' => 'TINYINT UNSIGNED',
			...
			'uploads' => array(
				...
				'content' => 'VARCHAR('.QA_DB_MAX_CONTENT_LENGTH.')',
				'format' => 'VARCHAR('.QA_DB_MAX_FORMAT_LENGTH.') CHARACTER SET ascii NOT NULL', // format e.g. 'jpeg', 'gif', 'png'
				'width' => 'INT UNSIGNED', // null means it's stored on disk in QA_BLOBS_DIRECTORY
				'height' => 'INT UNSIGNED', // name of source file (if appropriate)
				'created' => 'DATETIME', // when it was created
				'type' => 'VARCHAR(33)',
				'PRIMARY KEY (id)',
			),
			'poll' => array(
				...
				'postid' => 'INT UNSIGNED',
				'content' => 'VARCHAR('.QA_DB_MAX_CONTENT_LENGTH.')',
				'answers' => 'VARCHAR('.QA_DB_MAX_CONTENT_LENGTH.')',
				'extra' => 'VARCHAR(800)',
				'created' => 'DATETIME', // when it was created
				'PRIMARY KEY (id)',
			),
			...
			'userlimits' => array(
				'userid' => $useridcoltype.' NOT NULL',
				'action' => 'CHAR(1) CHARACTER SET ascii NOT NULL', // see constants at top of king-app-limits.php
...
/*
	Return the current version of the KINGMEDIA database, to determine need for DB upgrades
*/
	{
...
*/
	{
		...
		$oldprofileflags=array(
			...
			'facebook-f' => QA_FIELD_FLAGS_LINK_URL,
			'twitter' => QA_FIELD_FLAGS_LINK_URL,
			'instagram' => QA_FIELD_FLAGS_LINK_URL,
			'reddit' => QA_FIELD_FLAGS_LINK_URL,
			'pinterest-p' => QA_FIELD_FLAGS_LINK_URL,
			'linkedin-in' => QA_FIELD_FLAGS_LINK_URL,
...
*/
	{
		require_once QA_INCLUDE_DIR.'king-app/recalc.php';

		$definitions=qa_db_table_definitions();
		$keyrecalc=array();

	//	Write-lock all KINGMEDIA tables before we start so no one can read or write anything
	...
	//	Upgrade it step-by-step until it's up to date (do LOCK TABLES after ALTER TABLE because the lock can sometimes be lost)
		...
		while (1) {
			...
			//	Up to here: Version 1.4 developer preview
				...
				case 25:
					...
					if (isset($keycolumns['filename']))
						qa_db_upgrade_progress('Skipping upgrading blobs table since it was already upgraded by another KINGMEDIA site sharing it.');
				...
				case 34:
					if (!QA_FINAL_EXTERNAL_USERS) {
						...
						if (isset($keytables[qa_db_add_table_prefix('messages')]))
							qa_db_upgrade_progress('Skipping messages table since it was already added by another KINGMEDIA site sharing these users.');
				...
				case 48:
					if (!QA_FINAL_EXTERNAL_USERS) {
						...
						if (isset($keycolumns['type']))
							qa_db_upgrade_progress('Skipping upgrading messages table since it was already upgraded by another KINGMEDIA site sharing it.');
				...
				case 51:
					if (!QA_FINAL_EXTERNAL_USERS) {
						...
						if (isset($keycolumns['permit']))
							qa_db_upgrade_progress('Skipping upgrading userfields table since it was already upgraded by another KINGMEDIA site sharing it.');
				...
				case 52:
					if (!QA_FINAL_EXTERNAL_USERS) {
						...
						if (isset($keyindexes['created']))
							qa_db_upgrade_progress('Skipping upgrading users table since it was already upgraded by another KINGMEDIA site sharing it.');
			...
			//	Up to here: Verison 1.7.1
				case 60:
					qa_db_upgrade_query('ALTER TABLE ^posts CHANGE COLUMN postformat postformat '.$definitions['posts']['postformat']);
					qa_db_upgrade_query($locktablesquery);
					break;
				case 61:
					// Upgrade from alpha version removed
